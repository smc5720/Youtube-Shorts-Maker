...
- **`source`는 nullable 필수 필드다.** 필드를 통째로 빼면 "출처가 없는 입력 경로였다"와
  "생성기가 빠뜨렸다"가 구분되지 않는다. `--topic` 경로에는 출처가 없으므로 `null`이고,
  `--url` / `--text-file` 경로에서 값을 채우는 것은 #31이다.
...
_ROOT = Object(
    {
        ...
        # 출처 URL 또는 원문 출처 표기. 구조화된 출처 정보가 필요해지면 그것을 요구하는
        # 입력 경로(#31)가 필드를 늘린다 — 지금 모양을 정하면 쓰지 않는 계약이 굳는다.
