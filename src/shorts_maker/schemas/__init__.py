"""산출물 JSON 스키마 — `quiz.json` / `scenes.json` / `project.json`의 단일 진실 공급원.

세 파일은 생성 모듈과 렌더러, 편집 앱이 공유하는 계약이다. 필드명과 열거값은 문서가 아니라
**이 패키지가 확정한다** — 문서(퀴즈 스펙 3장, PRD 7.5.2, 7.10)는 여기 정의된 이름을 설명한다.
