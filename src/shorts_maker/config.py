...
SPEC: dict[str, Any] = {
    ...
    "source": {
        ...
        # 링크 입력만 읽는다 (#95). `--text-file` 실행에는 가져올 것도 추출할 것도 없어
        # 하위 섹션으로 갈라 둔다 — 위 두 값은 두 경로가 함께 쓴다.
        "url": {
            # 한 번의 가져오기에 거는 상한. LLM(600초)과 성격이 다르다 — 기사 페이지
            # 하나이고, 20초를 넘겼다면 기다릴 것이 아니라 링크나 네트워크를 의심할
            # 상황이다.
            "timeout_sec": Setting(20, "int"),
            # 응답 본문 상한. **표준 라이브러리 `urlopen`에는 크기 상한이 없다**
            # (스파이크 #31 5장) — 실측에서 위키백과 장문 한 페이지가 1.5MB였고, 그
            # 3배를 넘는 HTML은 기사가 아니라 잘못 지정한 링크에 가깝다.
            "max_bytes": Setting(5_000_000, "int"),
            # 리다이렉트 상한. 단축 URL·AMP 전환이 두어 번이고, 그보다 길게 도는 것은
            # 로그인 벽을 도는 순환에 가깝다.
            "max_redirects": Setting(5, "int"),
            # 추출된 본문의 하한. **목적은 "안내문 걸러내기"가 아니라 "빈 본문
            # 걸러내기"다** — 실측에서 로그인 안내문(800자)이 정상 기사(735자)보다 길었고,
            # 안내문을 거를 만큼 올리면 단신 기사가 함께 잘린다 (PRD 14.1). 300은 관측된
            # 가장 짧은 정상 기사(478자)와 JS 안내문(258자) 사이다.
            "min_chars": Setting(300, "int"),
        },
    },
    ...
    "audio": {
        ...
        # 배경음악 파일 (#35). **기본은 없음이고 번들 음악도 없다** (PRD 8장) — 경로를 손으로
        # 적는 것이 "라이선스를 확인했다"는 사용자의 확인 행위이고, 그래서 이 값에 기본
        # 후보를 두지 않는다. 받는 확장자는 백엔드가 소유한다
        # (`audio_mix.MUSIC_FILE_EXTENSIONS`).
        "music": Setting(None, "str", nullable=True),
        # 음악 트랙의 선형 게인. **1이 아닌 유일한 볼륨 기본값이다** — 낭독 아래로 깔리는
        # 트랙이고 사용자 파일이라 레벨을 알 수 없다. 0.30은 낭독 대비 약 -10dB 자리다.
        "music_volume": Setting(0.30, "float"),
        # 낭독 구간에서 음악에 곱하는 배수(0~1). 0.35는 약 -9dB이고, 대사 우선 믹스의
        # 통상 범위(8~12dB) 안이다 — 효과음이 낭독보다 9.5dB 아래인 것(#23 실측)과 같은
        # 자리를 노린 값이다.
        "music_duck": Setting(0.35, "float"),
        # 감쇠가 걸리고 풀리는 시간(초). **0으로 내리지 말 것** — 계단 전환은 클릭으로
        # 들리고 렌더가 그 값을 거부한다. 0.25초는 낭독 시작 앞의 `lead_in_sec`(0.30)보다
        # 짧아 첫 음절 전에 감쇠가 완성된다.
        "music_duck_fade_sec": Setting(0.25, "float"),
    },
