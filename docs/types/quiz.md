...
(`COUNTDOWN_SFX` / `ANSWER_SFX`)이고, 이름에서 경로를 찾는 것은 믹싱하는 #23이다. 규격은
mp3 / 48kHz / 스테레오이며 `beep` 1.0초 이하, `correct` 1.5초 이하, peak는 두 파일 모두
-12 dBFS 이하에 편차 3dB 이내다 (#18). 낭독 대비 최종 믹스 레벨은 #23이 정한다.
