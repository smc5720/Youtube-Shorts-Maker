...
남는다 (`--verbose`와 무관하다). **앱에서는 렌더 화면의 경고 목록이 그 자리이고 확인
체크박스가 게이트다** (#30) — `unverified`도 함께 오고, 사람이 확인한 항목
(`review.acknowledged`)은 빠진다. **판정은 앱이 하고 백엔드는 막지 않는다**: 백엔드가 막으면
CLI의 이 규칙과 갈린다 (PRD 14.1).
**기본 동작은 경고 후 진행이고 종료 코드는 0이다** —
...
- **앱에서 고친 것이 곧바로 영상에 반영되지는 않는다.** 순서·개수·낭독 문구를 고쳐도
  ...
  - **#77에서 들어왔다.** 앱의 재생성이 `scene_template` → 세그먼트(재사용/합성) → 타임라인 →
    `voice.mp3` → `captions.srt` → `project.json` 순서로 다시 만들고 낡음 표시를 지운다.
    **이 경로는 모델을 부르지 않는다** — 생성기도 메타데이터 생성기도 없으므로 사람이 고친
    `quiz.json`이 덮이지 않고, 그 대가로 `metadata.json`은 옛 제목·태그로 남는다 (PRD 14.1).
  - **`countdown_sec`을 고친 것이 장면에 도달하는 자리도 여기다.** 그 값은 콘텐츠 필드이고
    장면의 `seconds`·`duration`은 장면 템플릿이 옮겨 담으므로, 재생성 전에는 화면의 숫자와
    장면 길이가 갈려 있다.
