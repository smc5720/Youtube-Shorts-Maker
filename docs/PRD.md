...
- 사람이 고친 장면 길이를 `scenes.json`에 덮을지 `project.json`에 얹을지 결정해야 한다.
  덮으면 재실행이 실측으로 되돌리고, 얹으면 두 파일이 다른 길이를 말한다. (#29)
- 배경으로 받을 사용자 파일의 지원 형식을 정해야 한다. `background.kind`가 `image`/`video`를
  받는다는 것까지만 정해져 있다. (#29)
