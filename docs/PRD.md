...
- **사람의 `flagged` 확인은 `quiz.json`의 `verify`를 건드리지 않는다.** 시안은 확인 버튼이
  ...
  "검증 단계가 결함을 찾지 못했다"는 뜻이기 때문이다 (퀴즈 스펙 5.2). 기록 위치는 #28에서
  정하며, `project.json`에 확인한 문제 `id` 목록을 두는 쪽이 경계에 맞는다.
...
- 사람이 `flagged`를 확인한 기록을 어디에 남길지 결정해야 한다. `quiz.json`의 `verify`는
  검증기가 소유하므로 쓸 수 없다. (#28, 14.1의 D2 결정)
