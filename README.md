...
| 인자 | 기본값 | 설명 |
| --- | --- | --- |
| `--topic` | (셋 중 하나 필수) | 쇼츠로 만들 주제 한 줄 |
| `--text-file` | (셋 중 하나 필수) | 원문 텍스트 파일. 첫 비어 있지 않은 줄이 제목이 됨 |
| `--url` | (셋 중 하나 필수) | 본문을 추출할 링크. **아직 지원하지 않음** |
| `--type` | `quiz` | 쇼츠 타입 |
...
> 효과음이 함께 믹스됩니다. **아직 없는 것** — 배경음악과 ducking, 장면 모션, `--url`
> 입력입니다. 편집 앱은 셸까지 있습니다(아래). 설정 키 일부는 값을 읽어 둘 뿐 아직 소비하는
> 단계가 없습니다.
>
> **`--text-file`은 지금 원문을 기록만 합니다.** 콘텐츠 생성기에 가는 것은 제목이고 본문은
> `source.json`에만 남습니다 — 본문을 소비하는 요약·대본 단계가 아직 없어서이며, 그 사실이
> `run.log`에 남습니다.
