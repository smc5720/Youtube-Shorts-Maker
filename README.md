...
> 효과음이 함께 믹스됩니다. 이미지·영상 배경에는 `render.motion`으로 약한 zoom/pan을 걸 수
> 있습니다 — **기본은 꺼져 있고**, 프리셋·단색 배경에서는 결과가 같으므로 필터가 붙지
> 않습니다. `audio.music`에 파일을 지정하면 배경음악이 섞이고 **낭독 구간에서 자동으로
> 낮아집니다**(ducking) — 음악은 기본 제공하지 않으므로 경로를 직접 적어야 하고, 그 경로와
> 라이선스 확인 책임이 `run.log`에 남습니다. 편집
