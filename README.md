...
`ffprobe`로 재므로 **FFmpeg가 설치되어 PATH에 있어야 합니다.**
...
> **현재는 음성까지 나옵니다.** 퀴즈 타입은 주제에서 문제 세트를 만들고 블라인드 검증까지
> 마쳐 `quiz.json`을 남기고, 이어서 `scenes.json`·`metadata.json`과 낭독 장면별 세그먼트
> 오디오(`audio/seg-*.mp3`)를 만듭니다. 장면 길이 확정과 `voice.mp3`·자막·렌더는 아직
> 연결되지 않았습니다. `--url` / `--text-file` 입력은 이후 이슈에서 추가됩니다. 설정 키
> 일부도 값을 읽어 둘 뿐 아직 소비하는 단계가 없습니다.
