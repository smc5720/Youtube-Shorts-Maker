...
- **장면** (#26 셸 + #27) — 장면 목록 · 세로형 프리뷰 · 속성 패널 3분할. 장면을 고르면 그
  장면의 대표 프레임이 뜬다. 속성 패널에서 **장면 길이**(#82)와 **자막 스타일·배경
  프리셋**(#79)을 고칠 수 있고, 배경 파일(#80)·볼륨(#81)·자막 문구(#83)와 렌더 실행(#30)이
  그 자리에 이어 들어온다
...
- **프리셋 이름을 앱에 적지 않는다** (#79). 자막 스타일·배경 목록은 백엔드의 `presets`가
  `assets/`에서 읽어 보내고 색도 함께 온다 — 동결 배포에서 `assets/`는 백엔드 실행 파일 옆이라
  앱에서 본 경로와 다르다. `npm run smoke`가 앱 소스에 프리셋 이름이 있는지까지 본다
