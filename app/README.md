...
- **장면** (#26 셸 + #27) — 장면 목록 · 세로형 프리뷰 · 속성 패널 3분할. 장면을 고르면 그
  장면의 대표 프레임이 뜬다. 속성 패널에서 **장면 길이**(#82), **자막 문구와 텍스트
  오버레이**(#83), **자막 스타일·배경 프리셋**(#79), **배경 사용자 파일**(#80), **낭독·효과음
  볼륨**(#81)을 고칠 수 있고, 렌더 실행(#30)이 그 자리에 이어 들어온다
...
**낡음이 두 종류다** (#83). 자막 문구·해설처럼 낭독에 가지 않는 것만 바뀌면 **자막만
낡고**(사각 `↻`, `captions.srt`만 다시 만들면 된다), 낭독되는 문장이 바뀌면 **음성까지
낡는다**(원형 `♪`). 색은 둘 다 파랑이고 아이콘 모양과 문구로 갈린다.
...
app/
  ...
  smoke/run.mjs         스모크 오케스트레이션 (앱을 여덟 번 띄운다)
...
- **프리뷰와 렌더 스모크는 FFmpeg를 요구한다.** 실제 프레임과 실제 mp4가 그 시나리오들의
  절반이라 대역으로 바꾸면 확인하려는 것이 확인되지 않는다. 나머지 다섯은 FFmpeg 없이 돈다
...
- **`review`와 `audio`는 프리뷰 프레임에 닿지 않아 프리뷰를 다시 만들지 않는다.** 확인 버튼
  한 번(#28)이나 볼륨 슬라이더 한 칸(#81)이 프레임 전부를 다시 만들면 2초가 붙는데 결과는
  같은 그림이다 (`schemas/project.py`의 `PREVIEW_BLIND_SECTIONS`)
- **볼륨은 눈금이 아니라 선형 게인으로 저장된다** (#81). 슬라이더의 100이 게인 `1.0`이고,
  100을 넘겨도 최종 오디오는 리미터가 -1 dBFS에서 잡는다
...
- **오버레이의 후보 목록도 백엔드에서 온다** (#83, 같은 `presets` 응답의 `overlay`). 9칸 위치·
  색·크기·웨이트가 그것이고, 웨이트를 앱이 들면 시안이 적은 400·600이 저장될 수 있다 — 그때
  **화면은 정상이고 렌더만 멈춘다** (번들 Pretendard는 500/700/800이다)
- **사람이 얹은 편집은 전부 `project.json`에 있다** (#82, #83). 장면 길이·자막 문구·오버레이가
  `render.scene_overrides`의 한 항목(`role` + `question_id`)에 함께 붙고 `scenes.json`은 앱이
  쓰지 않는다 — 낭독보다 짧은 길이는 확정 검증이 거부하고, 문구·오버레이는 재생성(#77)이
  장면을 다시 만들 때 사라진다
...
- **렌더는 앱이 들고 있는 프로젝트로 돈다** (#30). 프리뷰와 같은 규칙이라 **저장하지 않은
  편집도 결과에 들어가고**, 그 사실이 렌더 화면의 경고 목록에 뜬다. 막는 것은 `flagged`·
  `unverified` 확인 하나뿐이고 낡음은 알리기만 한다 — 재생성(#77)이 없는 동안 막으면 렌더를
  아예 할 수 없다
- **진행률은 요청과 짝이 없는 알림으로 온다** (`onBackendEvent`). 백엔드가 보내는 것은 프레임
  수·총 프레임·장면 인덱스·경과 ms이고 **퍼센트와 남은 시간은 화면이 계산한다**
- **렌더 중 편집이 잠기는 판정은 `App.patchProject` 하나다.** 컨트롤을 비활성으로 그리는 것은
  표시일 뿐이라, 그것만으로는 메뉴·단축키·스모크가 잠금을 지나간다
- **렌더는 한 번에 하나이고 두 번째 요청은 백엔드가 `busy`로 거절한다.** 앱은 그것을 실패
  카드로 그리지 않는다 — 돌고 있는 렌더의 상태를 덮으면 화면이 "실패"를 말하는 동안 인코딩이
  계속 돈다
